...
const GuitarDiagram = ({ chordName, positions, fingers = [], noteStrip }: ChordDiagramProps) => {
  ...
  type Barre = { fret: number; startString: number; endString: number; finger: number };
  const barreGroups: Barre[] = (() => {
    ...
    byFret.forEach((arr, fret) => {
      ...
      while (runStart < sorted.length) {
        ...
        if (span >= 2) {
          groups.push({ fret, startString: sorted[runStart].string, endString: sorted[runEnd].string, finger });
        }
        runStart = runEnd + 1;
      }
    });
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6 max-w-md mx-auto" style={{ border: `4px solid ${theme.primary}`, background: '#fff' }}>
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex space-x-2">
          <button 
            ...
            className={`px-2 py-1 text-xs rounded ${orientation === 'normal' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
          >
            Normal
          </button>
          <button 
            onClick={() => setOrientation('left-handed')}
            className={`px-2 py-1 text-xs rounded ${orientation === 'left-handed' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
          >
            Left
          </button>
          <button 
            onClick={() => setOrientation('player-mirrored')}
            className={`px-2 py-1 text-xs rounded ${orientation === 'player-mirrored' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
          >
            Mirror
          </button>
        </div>
      ...
      <div className="relative mx-auto" style={{ width: '280px', height: '360px' }}>
        ...
        <div className="absolute inset-0" style={{ borderRadius: '10px', background: '#fff' }}>
          {/* Nut and end line */}
          {startFret === 1 && (
            <div className="absolute left-0 right-0" style={{ top: 0, height: '16px', background: '#000', zIndex: 12 }} />
          )}
          <div className="absolute left-0 right-0" style={{ bottom: 0, height: '6px', background: '#000', zIndex: 11 }} />
          ...
          {[...Array(frets)].map((_, i) => (
            <div
              key={`fret-${i+1}`}
              className="absolute"
              style={{ left: 0, right: 0, height: '2px', background: '#111', top: `calc((100%/6)*${i+1})`, zIndex: 10 }}
            />
          ))}

          {/* Inlays at 2.5 and 4.5 */}
          <div className="absolute" style={{ left: '50%', transform: 'translate(-50%, -50%)', width: '16px', height: '16px', borderRadius: 999, background: 'rgba(0,0,0,.12)', top: 'calc((100%/6)*2.5)', zIndex: 9 }} />
          <div className="absolute" style={{ left: '50%', transform: 'translate(-50%, -50%)', width: '16px', height: '16px', borderRadius: 999, background: 'rgba(0,0,0,.12)', top: 'calc((100%/6)*4.5)', zIndex: 9 }} />

          {/* Strings (columns 0..5) */}
          {[...Array(strings)].map((_, col) => {
            const stringNum = orientation === 'left-handed' ? col + 1 : strings - col;
            const widthPx = stringWidths[stringNum - 1];
            return (
              <div key={`string-${col}`} className="absolute" style={{ top: 0, bottom: 0, width: `${widthPx}px`, background: '#111', transform: 'translateX(-50%)', left: leftForCol(col), borderRadius: '2px', zIndex: 8 }}>
                {/* String label */}
                <div className="absolute -top-6 text-xs font-medium text-gray-700" style={{ left: '50%', transform: 'translateX(-50%)' }}>
                  {['E', 'A', 'D', 'G', 'B', 'E'][stringNum - 1]}
                </div>

                {/* XO indicators at top (only if start at nut) */}
                {startFret === 1 && (
                  <div className="absolute font-extrabold" style={{ color: theme.primary, top: 0, transform: 'translate(-50%, -100%)', left: '50%', fontSize: '32px', lineHeight: 1, zIndex: 15 }}>
          ...
          {positions.filter(p => p.fret > 0).map((pos, idx) => {
            ...
            const finger = fingers[idx] || undefined;
            return (
              <div key={`dot-${idx}`} className="absolute grid place-items-center text-white font-extrabold" style={{ width: '72px', height: '72px', borderRadius: 999, background: theme.primary, left: leftForCol(col), top: topForFret(pos.fret), transform: 'translate(-50%, -50%)', textShadow: '0 1px 0 rgba(0,0,0,.25)', zIndex: 20 }}>
          ...
          {barreGroups.map((b, i) => {
            ...
            const leftExpr = `calc((100%/${strings})*${Math.min(startCol, endCol)} + (100%/${strings})*0.12)`;
            const widthExpr = `calc((100%/${strings})*${Math.abs(endCol - startCol)} + (100%/${strings})*0.76)`;
            return (
              <div key={`barre-${i}`} className="absolute grid place-items-center text-white font-extrabold" style={{ height: '48px', borderRadius: 999, background: theme.primary, left: leftExpr, width: widthExpr, top: topForFret(b.fret), transform: 'translateY(-50%)', textShadow: '0 1px 0 rgba(0,0,0,.25)', zIndex: 19 }}>
      ...
      {noteStrip && noteStrip.length === 6 && (
        <div className="mt-3 grid grid-cols-6 gap-0 font-extrabold" style={{ fontSize: '20px', letterSpacing: '.5px' }}>
          {noteStrip.map((n, i) => (
            <span key={`ns-${i}`} className="text-center" style={{ color: theme.primary }}>{n}</span>
          ))}
        </div>
      )}
      
      <div className="mt-4 text-center">
        <button className="px-4 py-2 rounded-lg transition-colors text-white" style={{ background: theme.primary }}>
          Play Chord
        </button>
      </div>
    </div>
